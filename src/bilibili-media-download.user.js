...
import { signWRIDParams } from './lib/bilibili-wrid.js';
...
const downloadMediaData = async (II, box) => {
	...
	try {
		...
		G.info('download-media', '✔', II.nameLog);
...
const getDash = async (bvid, cid) => {
	const url = new URL('https://api.bilibili.com/x/player/wbi/playurl');

	const paramsWRIDSigned = signWRIDParams({ bvid, cid, fnval: '4048' });
	for(const name in paramsWRIDSigned) {
		url.searchParams.append(name, paramsWRIDSigned[name]);
	}
	...
	return new Promise((resolver, rejecter) => {
		...
		request.addEventListener('loadend', (e) => resolver(request.response?.data?.dash));
		request.addEventListener('error', (e) => rejecter(e));
...
const download = async (p_, isCloseAfterDownload) => {
	...
	const source = await getDash(state.bvid, String(state.cid));
	...
	G.info('picked video', video);
	G.info('picked audio', audio);
...
(async () => {
	try {
		...
		if(localStorage.getItem(`${namePackage}/auto-start`)) {
			download();
		}
	}
